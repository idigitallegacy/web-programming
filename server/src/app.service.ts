...
export class AppService {
  ...
  async getIndexPageContent() {
    ...
    // const posts = await postsController.findRange(0, postsPerPage)
    //
    // for (let offset = 0; offset < Math.min(postsAmount, postsPerPage); ++offset) {
    //   let postId = posts[offset].id
    //   let postTitle = posts[offset].title
    //   let postDate = posts[offset].date
    //   let postAuthor = await usersController.findById(posts[offset].author_id)
    //   let postPictureLink = posts[offset].picture_link
    //   let postBody = posts[offset].body
    //   content += "<div class=\"card\" data-id=\"" + postId + "\">\n" +
    //     ".card[data-id=\"" + postId + "\"]:before {background-image: black;}" +
    //     "<div class=\"content\">" +
    //     "      <h3 class=\"main-content-item-header\">" + postTitle + "</h3></h3>\n" +
    //     "      <p class=\"copy\">" + postBody.substring(0, 200) + "</p>\n" +
